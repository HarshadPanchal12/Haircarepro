...
app.get('/login', (req, res) => res.render('login', { user: req.session.user, error: null }));
app.get('/register', (req, res) => res.render('register', { user: req.session.user, error: null }));
app.get('/survey', (req, res) => {
  ...
  res.render('survey', { user: req.session.user, username: req.session.user.username, error: null });
});
...
app.post('/register', async (req, res) => {
  ...
  if (!validator.isEmail(email)) {
    return res.render('register', { error: "Invalid email format.", user: req.session.user });
  }

  const existing = await User.findOne({ $or: [{ username }, { email }] });
  if (existing) {
    return res.render('register', { error: "Username or email already exists.", user: req.session.user });
  }
...
app.post('/survey', async (req, res) => {
  ...
  pdf.fontSize(14).text(`For: ${req.session.user.username}`, { align: "center" });
  pdf.moveDown();

  // Handle different ingredient formats
  console.log("Ingredients data before PDF:", JSON.stringify(carePlan.ingredients, null, 2));
  if (carePlan.ingredients && Array.isArray(carePlan.ingredients)) {
    if (carePlan.ingredients.length > 0 && typeof carePlan.ingredients[0] === 'object' && 'name' in carePlan.ingredients[0]) {
      ...
      carePlan.ingredients.forEach(ing => {
        ...
        const howToUse = carePlan.instructions?.[ing] || 'No instructions available';
        pdf.text(`- ${name}: ${howToUse}`);
      });
    }
  } else {
    console.warn("Ingredients is not an array or is undefined:", carePlan.ingredients);
